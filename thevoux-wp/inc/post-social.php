...
function thb_fbLikeCount($pageID, $debug = false) {
	...
	if ( empty( $cache ) ) {
		...
		$json_url = 'https://graph.facebook.com/'.$pageID.'?access_token='.$thb_access_token.'&fields=fan_count';
		$json = wp_remote_get($json_url, array('timeout' => 10));
		// Check for error
		if ( is_wp_error( $json ) ) {
    ...
    if ( empty( $cache ) ) {
	    $request = wp_remote_get( 'https://api.instagram.com/v1/users/' . $id . '?access_token=' . $api_key );
			...
			if ( is_wp_error( $request ) ) {
				...
				echo $error_string = $request->get_error_message();
				return;
			}
			...
			if ( isset( $response->data ) && isset( $response->data->counts ) && isset( $response->data->counts->followed_by ) ) {
			  ...
			  set_transient( 'thb_page_inscount', $followers, $time );
				...
				echo esc_html($response->meta->error_message);
			}
