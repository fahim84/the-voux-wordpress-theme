...
.header.fixed .logo .logoimg {
	max-height: <?php thb_measurementecho(ot_get_option('logo_height_fixed')); ?>;
}
/* Title Type */
<?php if($title_type = ot_get_option('title_type')) { ?>
h1, h2, h3, h4, h5, h6, .mont, .post .post-author:not(.style2) em, .wpcf7-response-output, label, .select-wrapper select, .wp-caption .wp-caption-text, .smalltitle, .toggle .title, q, blockquote p, cite, table tr th, table tr td, #footer.style3 .menu, #footer.style2 .menu, .product-title, .social_bar, .widget.widget_socialcounter ul.style2 li, .post.listing.listing-style2 .just-shares span {
...
.widget.style5>strong span:after {
	border-left-color: <?php echo esc_attr($accent_color); ?>;
}
.widget.style5>strong span:before {
	border-right-color: <?php echo thb_adjustColorLightenDarken($accent_color, 10); ?>;
}
blockquote:before,
blockquote:after {
