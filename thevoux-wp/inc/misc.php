...
function thb_social() {
 ?>
	...
	<a href="<?php echo esc_url(ot_get_option('googleplus_link')); ?>" class="google-plus icon-1x social" target="_blank"><i class="fa fa-google-plus"></i></a>
	<?php } ?>
	...
	<a href="<?php echo esc_url(ot_get_option('youtube_link')); ?>" class="youtube icon-1x social" target="_blank"><i class="fa fa-youtube"></i></a>
	<?php } ?>
...
add_action('thb_categories', 'thb_categories');
...
function thb_catcolorecho($array) {
	if(!empty($array)) {
		foreach ($array as $cat => $color) {
			...
			if ($cat) {
				echo ".post-meta.style1 a.cat-".$cat->slug." { color: ". $color ."; }\n";
				echo ".post-meta.style2 a.cat-".$cat->slug." { background-color: ". $color ."; }\n";
